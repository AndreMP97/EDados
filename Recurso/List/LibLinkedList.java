public class LibLinkedList {
  ...
  T get(LinkedList<T> l, int index) {
    ...
    public static <T extends Comparable<? super T>>
    LinkedList<Integer> filterGreaterThan(LinkedList<Integer> l, Integer val) {
        LinkedList<Integer> n = new LinkedList<Integer>();
        Node<Integer> p = l.first;
        if (n.size == 0) {
	while (p.value >= val) {
	    if (p.next == null) {
	        return n;
	...
	n.first = new Node<Integer>(p.value, n.first);
	n.size++;
	p = p.next;
        }
        Node<Integer> k = n.first;
        while (p.next != null) {
	if (p.value < val) {
	    k.next = new Node<Integer>(p.value);
	    n.size++;
	    k = k.next;
	}
	p = p.next;
    ...
    public static <T extends Comparable<? super T>>
        LinkedList<T> reverse(LinkedList<T> l) {
