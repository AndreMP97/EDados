...
class LibBSTree<K extends Comparable<? super K>,V> {
  ...
  int keysLessThan(BSTNode<K,V> n, K key) {
    ...
    public static <K extends Comparable<? super K>, V>
	int keysGreaterThan(BSTree<K,V> t, K key) {
	return keysGreaterThan(t.root, key);
    }

    private static <K extends Comparable<? super K>,V>
	int keysGreaterThan(BSTNode<K,V> n, K key) {
	if (n == null)
	    return 0;
	if ((n.key).compareTo(key) > 0)
	    return 1 + size(n.right) + keysGreaterThan(n.left,key);
	else
	    return keysGreaterThan(n.right, key);
  ...
  boolean isBalanced(BSTNode<K,V> n) {
    ...
    //Exercicio 5
    public static <K extends Comparable<? super K>,V>
	K minimum(BSTree<K,V> t) {
	return minimum(t.root);	   
    }

    private static <K extends Comparable<? super K>,V>
	K minimum(BSTNode<K,V> n) {
	if (n.left == null)
	    return n.key;
	else
    ...
    //Exercicio 5
    public static <K extends Comparable<? super K>,V>
	K maximum(BSTree<K,V> t) {
	return maximum(t.root);
    }

    private static <K extends Comparable<? super K>,V>
	K maximum(BSTNode<K,V> n) {
	if (n.right == null)
	    return n.key;
	else
    ...
    //Exercicio 6
    public static <K extends Comparable<? super K>,V>
	int minDepth(BSTree<K,V> t) {
	return minDepth(t.root);
    }

    private static <K extends Comparable<? super K>,V>
	int minDepth(BSTNode<K,V> n) {
	int min;
	if (depth(n.left) != 0 && depth(n.right) != 0) {
	    min = Math.min(depth(n.left), depth(n.right));
	}
	else if (depth(n.left) == 0) {
	    min = depth(n.right);
	}
	else if (depth(n.right) == 0) {
	    min = depth(n.left);
	}
	else {
	    min = 0;
    ...
    //Exercicio 7
    /*public static <K extends Comparable<? super K>,V>
	int lastlevel(BSTree<K,V> t) {
	return lastlevet(t.root);
    }

    private static <K extends Comparable<? super K>,V>
	int lastlevel(BSTNode<K,V> n) {
